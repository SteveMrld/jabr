...
// --- DASHBOARD ---
const DashboardView = ({ onProject, onNew }: { onProject: (p: Project) => void; onNew: () => void }) => {
  ...
  return (
    <div>
      ...
      <div className="flex gap-3.5 mb-7 flex-wrap">
        <StatCard value={PROJECTS.length} label="Projets" accent={c.mv} />
        <StatCard value={prog} label="En cours" accent={c.og} />
        <StatCard value={pub} label="Publiés" accent={c.ok} />
        <StatCard value="37/100" label="ISBN attribués" accent={c.or} />
      ...
      <Card hover={false}>
        <div className="flex justify-between items-center px-5 py-3.5" style={{ borderBottom: `2px solid ${c.or}` }}>
          ...
          <span style={{ fontSize: 11, color: c.gr }}>{PROJECTS.length} titres</span>
        </div>
        {PROJECTS.map(p => (
...
// --- COUVERTURES ---
const CouverturesView = ({ onProject }: { onProject: (p: Project) => void }) => {
...
// --- COLLECTIONS ---
const CollectionsView = ({ onProject }: { onProject: (p: Project) => void }) => (
  <div>
...
const NewProjectModal = ({ open, onClose }: { open: boolean; onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      ...
      <div onClick={e => e.stopPropagation()} className="relative bg-white rounded-2xl w-[500px] shadow-2xl">
        ...
        <div className="p-6">
          ...
          <div className="rounded-lg p-3.5 mb-5" style={{ background: c.ft }}>
            ...
            <div className="font-semibold text-[15px]" style={{ fontFamily: "'JetBrains Mono', monospace", color: c.mv }}>978-2-488647-34-2</div>
            <div className="text-[11px] mt-0.5" style={{ color: c.gr }}>Prochain ISBN disponible dans le stock</div>
          </div>
          <div className="flex gap-3 justify-end">
            <Btn variant="secondary" onClick={onClose}>Annuler</Btn>
            <Btn onClick={onClose}>{icons.plus} Créer le projet</Btn>
          </div>
        </div>
      </div>
    </div>
...
export default function JabrApp() {
  ...
  const renderContent = () => {
    ...
    switch (page) {
      case 'projets': return <DashboardView onProject={openProject} onNew={() => setModalOpen(true)} />;
      case 'couvertures': return <CouverturesView onProject={openProject} />;
      case 'isbn': return <ISBNView />;
      case 'collections': return <CollectionsView onProject={openProject} />;
      case 'analytics': return <AnalyticsView />;
      case 'distribution': return <DistributionView />;
      case 'calibrage': return <PlaceholderView icon="📐" title="Calibrage" desc="Importez un manuscrit (.docx) et JABR calculera automatiquement le nombre de pages, le format, la police et les marges." btn="Importer un manuscrit" />;
      case 'audiobooks': return <PlaceholderView icon="🎧" title="Audiobooks" desc="Génération vocale IA, clonage de voix, mastering automatique, distribution multi-plateforme." btn="Lancer une production" />;
      case 'settings': return <PlaceholderView icon="⚙️" title="Paramètres" desc="Configuration de Jabrilia Éditions : charte graphique, format auteur, logo standard, préfixe ISBN." />;
      default: return <DashboardView onProject={openProject} onNew={() => setModalOpen(true)} />;
    }
  };
  ...
  return (
    <div className="flex min-h-screen" style={{ fontFamily: "'Inter', sans-serif", background: c.bc }}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex justify-end items-center px-8 py-2.5 bg-white" style={{ borderBottom: `1px solid ${c.gc}` }}>
